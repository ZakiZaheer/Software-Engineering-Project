...
class EventModificationScreen extends StatefulWidget {
  final Event event;
  const EventModificationScreen({super.key , required this.event});
  @override
  State<EventModificationScreen> createState() => _EventModificationScreenState();
}

class _EventModificationScreenState extends State<EventModificationScreen> {
  late Event event;
  bool isAllDay = false;
  ...
  void initState() {
    ...
    _selectedReminderType = event.reminders != null ? event.reminders![0].reminderType : "Default";
    if(_selectedStartTime!.hour == 0 && _selectedStartTime!.minute == 0 && _selectedEndTime == _selectedStartTime!.add(Duration(days: 1))){
      isAllDay = true;
    }
    super.initState();
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          CustomSlider(
            initialValue: event.reminders == null ? 0 : _checkReminderType(event.reminders![0]),
      ...
      if (repeatPattern == "Never") {
        ...
        setState(() {
          if(event.repeatPattern == null){
            ...
            event.repeatPattern!.repeatInterval = newRepeatPattern.repeatInterval;
            event.repeatPattern!.repeatUnit = newRepeatPattern.repeatUnit;
            event.repeatPattern!.repeatOn = newRepeatPattern.repeatOn;
          }
        });
      }
    }
...
double _checkReminderType(EventReminder reminder){
